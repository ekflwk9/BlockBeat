...
public static class FirebaseManager
{
    ...
    private static async void LoadLeaderboard()
    {
        ...
        score = new int[values.ChildrenCount];
        ...
        while (enumerator.MoveNext())
        {
            ...
            score[index] = System.Convert.ToInt32(currentUser.Child(scoreName).Value);
    ...
    public static void Add()
    {
        ...
        var scorePath = $"{child}/{scoreName}";

        data.Child(userPath).SetValueAsync(Json.GetName());          //플레이어 이름을 파이어 베이스에 올림
        data.Child(scorePath).SetValueAsync(Json.GetPlayMaxScore()); //최고 점수 파이어 베이스에 올림
    }
}